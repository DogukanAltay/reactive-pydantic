...
class TestPerformance:
    ...
    def test_basic_performance(self):
        ...
        TestModel.observe_field("value").pipe(
            map_to_value()
        ...
        assert modification_time < 0.5, f"Modification took too long: {modification_time:.3f}s"
        assert len(events) == 1000, f"Expected 1000 events, got {len(events)}"
        
        # Log performance metrics for visibility
        print(f"\nPerformance Metrics:")
        print(f"  Created 1000 models in {creation_time:.3f}s ({creation_time*1000:.1f}ms)")
        print(f"  Modified 1000 models in {modification_time:.3f}s ({modification_time*1000:.1f}ms)")
    ...
    def test_subscription_performance(self):
        ...
        for i in range(10):
            TestModel.observe_field("value").subscribe(
                lambda val, counter=i: event_counters.__setitem__(counter, event_counters[counter] + 1)
            )
        ...
        assert elapsed_time < 0.5, f"Multi-subscription processing took too long: {elapsed_time:.3f}s"
        assert sum(event_counters) == 1000, f"Expected 1000 total events, got {sum(event_counters)}"
        ...
        for i, count in enumerate(event_counters):
            assert count == expected_events_per_subscription, (
    ...
    def test_memory_usage(self):
        ...
        max_retained = max(2000, objects_added * 1.2)  # Allow up to 120% retention or 2000 objects
        ...
        retention_rate = (objects_remaining / objects_added) * 100 if objects_added > 0 else 0
    ...
    def test_field_change_throughput(self):
        ...
        # Subscribe to changes
        TestModel.observe_field("name").subscribe(lambda event: events.append(event.new_value))
        ...
        assert throughput > 10000, f"Throughput too low: {throughput:.0f} changes/second"
    ...
    def test_concurrent_model_creation(self):
        ...
        TestModel.observe_field("value").subscribe(lambda event: all_events.append(f"value:{event.new_value}"))
        TestModel.observe_field("name").subscribe(lambda event: all_events.append(f"name:{event.new_value}"))
        ...
        for i in range(100):
            model = TestModel(value=i, name=f"Model_{i}")
            model.value = i * 2 + 1000  # Trigger additional event (ensure different value)
        ...
        assert elapsed_time < 0.5, f"Concurrent operations took too long: {elapsed_time:.3f}s"
