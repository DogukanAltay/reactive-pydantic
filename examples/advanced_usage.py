...
def demonstrate_buffered_observation():
    ...
    User.observe_field("email").pipe(
        buffer_changes(3),
        ops.map(lambda buffer: [event.new_value for event in buffer])
...
def demonstrate_filtering():
    ...
    ).subscribe(lambda event: field_changes.append(f"Field changed: {event.field_name}"))
    
    User.observe_model().pipe(
        where_event_type(EventType.VALIDATION_SUCCESS)
    ).subscribe(lambda event: validation_events.append("Validation success"))
