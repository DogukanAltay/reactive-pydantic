...
from reactive_pydantic.operators import buffer_changes, debounce_changes
...
def test_debounced_field_changes():
    ...
    Product.observe_field("price").pipe(debounce_changes(0.1)).subscribe(events.append)
...
def test_buffered_changes():
    ...
    Product.observe_field("quantity").pipe(buffer_changes(3)).subscribe(events.append)
