...
class TestComprehensiveFunctionality:
    ...
    def test_basic_functionality(self):
        ...
        events = []
        User.observe_field("name").pipe(map_to_value()).subscribe(
    ...
    def test_distinct_until_changed(self):
        ...
        User.observe_field("age").pipe(
            ops.distinct_until_changed(), map_to_value()
    ...
    def test_where_field_operator(self):
        ...
        field_events = []
        User.observe_model().pipe(where_field("email"), map_to_value()).subscribe(
            lambda val: field_events.append(val)
        )
    ...
    def test_debounce_changes_operator(self):
        ...
        User.observe_field("name").pipe(
            debounce_changes(0.05), map_to_value()
    ...
    def test_buffer_changes_operator(self):
        ...
        buffered_events = []
        User.observe_field("tags").pipe(buffer_changes(2)).subscribe(
            lambda events: buffered_events.append(len(events))
        )
    ...
    def test_multiple_subscriptions(self):
        ...
        User.observe_field("name").subscribe(
            lambda event: events1.append(event.new_value)
        )
        User.observe_field("name").subscribe(
            lambda event: events2.append(event.new_value)
        )
    ...
    def test_event_type_filtering(self):
        ...
        User.observe_model().pipe(where_event_type(EventType.FIELD_CHANGED)).subscribe(
            lambda event: field_change_events.append(event.field_name)
        )
    ...
    def test_model_serialization(self):
        ...
        user = User(
            name="Alice", age=25, email="alice@example.com", tags=["tag1", "tag2"]
        )
    ...
    def test_error_handling_in_subscriptions(self):
        ...
        User.observe_field("name").subscribe(on_next=error_prone_handler)
    ...
    def test_nested_model_reactivity(self):
        ...
        events = []
        User.observe_field("tags").subscribe(
            lambda event: events.append(len(event.new_value))
        )
    ...
    def test_concurrent_field_modifications(self):
        ...
        User.observe_field("name").subscribe(
            lambda event: name_events.append(event.new_value)
        )
        User.observe_field("age").subscribe(
            lambda event: age_events.append(event.new_value)
        )
